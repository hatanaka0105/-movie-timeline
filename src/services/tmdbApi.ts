...
// モックデータ（デモ用）
function getMockSearchResults(query: string): Promise<TMDbMovie[]> {
  const mockMovies: TMDbMovie[] = [
    {
      ...
      overview: '1912年、豪華客船タイタニック号の処女航海。貧しい青年ジャックと上流階級の娘ローズの運命的な恋の物語。',
      poster_path: null,
      backdrop_path: null,
      genre_ids: [18, 10749],
      vote_average: 7.9,
    ...
    },
    {
      ...
      overview: '2049年、新型レプリカントのKは旧型レプリカントを追う任務に就いている。',
      poster_path: null,
      backdrop_path: null,
      genre_ids: [878, 53],
      vote_average: 8.0,
    ...
    },
    {
      ...
      overview: '第二次世界大戦中、ナチスドイツによるユダヤ人迫害の中、1100人以上のユダヤ人を救った実業家オスカー・シンドラーの実話。',
      poster_path: null,
      backdrop_path: null,
    ...
    },
    {
      ...
      overview: '1985年の高校生マーティが、科学者ドクの発明したタイムマシンで1955年にタイムスリップしてしまう。',
      poster_path: null,
      backdrop_path: null,
      genre_ids: [878, 12, 35],
      vote_average: 8.3,
  ...
  return Promise.resolve(filtered.length > 0 ? filtered : mockMovies.slice(0, 2));
}

export function getMockMovieDetails(movieId: number): Promise<TMDbMovieDetails | null> {
  const mockMovies: Record<number, TMDbMovieDetails> = {
    1: {
      id: 1,
      title: 'タイタニック',
      original_title: 'Titanic',
  ...
  return Promise.resolve(mockMovies[movieId] || null);
}
